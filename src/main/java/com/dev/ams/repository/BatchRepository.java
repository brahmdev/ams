...
public interface BatchRepository extends CrudRepository<Batch, String> {
    ...
    @Query(value = "SELECT b FROM Batch b WHERE b.standard.id IN (\n" +
            ...
            "\t\tSELECT id FROM Board b WHERE b.institute.id IN (\n" +
            "\t\t\tSELECT id FROM Institute i WHERE i.id = :instituteId\n" +
            "        )\n" +
            "    )\n" +
            ")")
    Iterable<Batch> findAllBatchByInstituteId(Integer instituteId);
}
